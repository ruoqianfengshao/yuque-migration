...
export async function main(url: string, options: ICliOptions) {
  ...
  setConfig({ ...options, host: `${newUrl.protocol}//${newUrl.host}`, url })
  if (!isValidUrl(url)) {
    throw new Error('Please enter a valid URL')
  }
  const { bookId, tocList, bookName, bookDesc, bookSlug, host, imageServiceDomains } =
    await getKnowledgeBaseInfo(url)
  if (host) {
    const secondDomain = host.split('.').slice(1).join('.')
    setConfig({ secondDomain })
  }

  if (!bookId) throw new Error('No found book id')
  if (!tocList || tocList.length === 0) throw new Error('No found toc list')
  const bookPath = path.resolve(options.distDir, bookName ? bookName : String(bookId))
