...
export async function downloadArticle(params: DownloadArticleParams): Promise<boolean> {
  ...
  const articleTitle = fixPath(originArticleTitle)
    ...
    if (options.sheetExportType === SheetExportType.lakesheet && id) {
      try {
        ...
        const newPath = progressItem.path
          .split('/')
          .slice(0, -1)
          .concat(`${fixPath(articleTitle)}.lakesheet`)
          .join('/')
        progressItem.path = newPath
        progressItem.savePath = saveFilePath
          .split('/')
          .slice(0, -1)
          .concat(`${fixPath(articleTitle)}.lakesheet`)
          .join('/')
    ...
    if (options.boardExportType === BoardExportType.lakeboard && id) {
      try {
        ...
        const newPath = progressItem.path
          .split('/')
          .slice(0, -1)
          .concat(`${fixPath(articleTitle)}.lakeboard`)
          .join('/')
        progressItem.path = newPath
        progressItem.savePath = saveFilePath
          .split('/')
    ...
    if (options.tableExportType === TableExportType.laketable && id) {
      try {
        ...
        const newPath = progressItem.path
          .split('/')
          .slice(0, -1)
          .concat(`${fixPath(articleTitle)}.laketable`)
          .join('/')
        progressItem.path = newPath
        progressItem.savePath = saveFilePath
          .split('/')
  ...
  } else if (ARTICLE_CONTENT_TYPE.DOC === contentType) {
    // 文档类型
    if (options.docExportType === DocExportType.lake && id) {
      try {
        await exportLakeFile({
          id,
          articleTitle,
        ...
        const newPath = progressItem.path
          .split('/')
          .slice(0, -1)
          .concat(`${fixPath(articleTitle)}.lake`)
          .join('/')
        progressItem.path = newPath
        progressItem.savePath = saveFilePath
          .split('/')
  ...
  const attachmentsErrInfo: string[] = []
  ...
  try {
    ...
    if (options.docExportType === DocExportType.md) {
      await writeFile(saveFilePath, handleMdData(mdData, handleMdDataOptions))
    }
