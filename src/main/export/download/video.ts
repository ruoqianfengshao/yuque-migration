...
export async function downloadVideo(params: IDownloadVideo) {
  ...
  let videoLinkList: ILinkItem[] = []
  const videoCards = $('card[name="video"]')
  let astTree: Root = { type: 'root', children: [] }
  try {
    // 查找所有 video 类型的 card
    videoCards.each((index, element) => {
      ...
      try {
        if (value) {
          const data = encodeURIComponent(decodeURIComponent(value).slice(5))
          const node = {
            node: {
              url: `https://www.yuque.com?_lake_card=${data}`,
              type: 'link',
              children: []
            } as Link,
            keyChain: []
          }
          videoLinkList.push(node)
        }
      } catch (error) {
        console.log(error)
      }
    ...
    astTree = getAst(mdData)
    const linkList = getLinkList(astTree)
    videoLinkList = linkList.filter((link) => /_lake_card.*?videoId/.test(link.node.url))
  }
